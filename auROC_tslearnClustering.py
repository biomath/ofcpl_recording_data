...
# Session you want to cluster; keep in mind passive sessions do not contain true trial outcomes or spout events and
...
for cur_col in col_name:
    ...
    # Save gap-stat data
    gapstat_df.to_csv(sep.join([OUTPUT_FOLDER, cur_col + '_' + which_session + '_gapStat.csv']), index=False)

    print("clusters:", k)

    with PdfPages(sep.join([OUTPUT_FOLDER, cur_col + '_' + which_session + '_gapStat.pdf'])) as pdf:
        fig, ax = plt.subplots()
    ...
    with PdfPages(sep.join([OUTPUT_FOLDER, cur_col + '_' + which_session + '_HClustering.pdf'])) as pdf:
        plt.figure()
    ...
    # Save data
    df.to_csv(sep.join([OUTPUT_FOLDER, cur_col + '_' + which_session + '_HClustering.csv']), index=False)

    # Plot average+-95%CI responses per cluster
    with PdfPages(sep.join([OUTPUT_FOLDER, cur_col + '_' + which_session + '_meanResponse.pdf'])) as pdf:
        fig, ax = plt.subplots()
